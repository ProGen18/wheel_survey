...
export function buildReferralLink(code, reqOrBase) {
  let base = '';
  if (typeof reqOrBase === 'string') {
    base = reqOrBase.replace(/\/$/, '');
  } else if (reqOrBase && typeof reqOrBase.headers?.get === 'function') {
    const host = reqOrBase.headers.get('host') || '';
    const proto = reqOrBase.headers.get('x-forwarded-proto') || 'https';
    base = `${proto}://${host}`;
  } else {
